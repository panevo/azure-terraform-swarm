resource "azurerm_resource_group" "swarm_cluster" {
  name     = "${var.product_key}-${local.environment_key}-${local.location_key}-swarm-rg"
...
resource "azurerm_virtual_network" "swarm_cluster_vnet" {
  name                = "${var.product_key}-${local.environment_key}-vnet-${local.location_key}${var.name_postfix}"
...
resource "azurerm_subnet" "node_subnet" {
  name                 = "${var.product_key}-${local.environment_key}-subnet-backend-${local.location_key}${var.name_postfix}"
  resource_group_name  = azurerm_resource_group.swarm_cluster.name
...
resource "azurerm_availability_set" "vm_availabilityset_manager" {
  name                         = "${var.product_key}-${local.environment_key}-availabilityset-${local.location_key}${var.name_postfix}"
  resource_group_name          = azurerm_resource_group.swarm_cluster.name
  location                     = azurerm_resource_group.swarm_cluster.location
  tags                         = local.tags
...
resource "azurerm_availability_set" "vm_availabilityset_workers" {
  name                         = "${var.product_key}-${local.environment_key}-availabilityset-worker-${local.location_key}${var.name_postfix}"
  resource_group_name          = azurerm_resource_group.swarm_cluster.name
  location                     = azurerm_resource_group.swarm_cluster.location
  tags                         = local.tags
  platform_update_domain_count = 5
...
resource "azurerm_network_security_group" "manager_sg" {
  name                = "${var.product_key}-${local.environment_key}-nic-sg-managers-${local.location_key}${var.name_postfix}"
  resource_group_name = azurerm_resource_group.swarm_cluster.name
  location            = azurerm_resource_group.swarm_cluster.location
...
resource "azurerm_network_security_group" "worker_sg" {
  name                = "${var.product_key}-${local.environment_key}-nic-sg-workers-${local.location_key}${var.name_postfix}"
  resource_group_name = azurerm_resource_group.swarm_cluster.name
  location            = azurerm_resource_group.swarm_cluster.location
